...
def resident_login():
    ...
    if resident:
        ...
        return redirect(url_for('login'))  # Make sure 'login' is the endpoint for the login page.
...
def view_requests():
    ...
    maintenance_requests = MaintenanceRequest.query.order_by(MaintenanceRequest.id).all()
...
def filter_requests(request_type=None):
    requests = {'travel': TravelRequest, 'maintenance': MaintenanceRequest, 'doctor': DoctorRequest}
    
    if request_type and request_type in requests:
        filtered_requests = requests[request_type].query.all()
        return render_template('all_requests.html', requests=filtered_requests, request_type=request_type)
    else:
        return all_requests()
...
def all_requests():
    ...
    return render_template('all_requests.html', travel_requests=travel_requests, maintenance_requests=maintenance_requests)
...
def edit_request(request_id):
    ...
    request_details = travel_request_details if travel_request_details else maintenance_request_details
    ...
    if request.method == 'POST':
        ...
        if is_maintenance_request:
            ...
            if date_completed:
                request_details.date_completed = datetime.strptime(date_completed, '%Y-%m-%d').date()
        ...
        return render_template('edit_request.html', request=request_details, is_maintenance_request=is_maintenance_request)
...
def edit_maintenance_request(request_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('edit_request.html', request=request_details, is_maintenance_request=True)
